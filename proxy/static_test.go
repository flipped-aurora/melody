...
func TestNewStaticDataMiddleware_multipleNext(t *testing.T) {
	...
	mw := NewStaticDataMiddleware(&endpoint)
	mw(explosiveProxy(t), explosiveProxy(t))
}

func TestNewStaticDataMiddleware_ok(t *testing.T) {
	endpoint := config.EndpointConfig{
		ExtraConfig: config.ExtraConfig{
			Namespace: map[string]interface{}{
				staticKey: map[string]interface{}{
...
func Test_getStaticConfig_ko(t *testing.T) {
	for i, cfg := range []config.ExtraConfig{
		...
		if _, ok := getStaticConfig(cfg); ok {
			t.Errorf("expecting error on test #%d", i)
		}
	}
}

func Test_getStaticConfig_strategy(t *testing.T) {
	for _, strategy := range []string{
		...
		staticCfg, ok := getStaticConfig(cfg)
		if !ok {
